            ...
            @if (isset($sortedData[$day][$StandTemplate->id]) && count($sortedData[$day][$StandTemplate->id]) > 0)
                ...
                <div class="card card-header card-header-content-between rounded text-center" style="background: rgba(153,102,204,0.5)">
                    <div class="flex-grow-1">
                        <p class="card-header-title h4 dd">
...
{{--                           href="{{ route('stand.record_redaction', ['stand_publishers_id'=> $standPublisher->id]) }}"--}}
                         ...
                         {{ isset($publishers['user_1']) && isset($publishers['user_2']) && $publishers['user_1'] && $publishers['user_2'] && (!$standPublisher || $canEdit) ? 'is-editable' : '' }}"
                               @if($standPublisher && $canEdit)
                                   onclick="openModalRedaction('{{$day}}', '{{$time}}', '{{$entry['date']}}', '{{$standPublisher->id}}','{{$StandTemplate->id}}')"
                               {{--                           href="{{ route('stand.record_redaction', ['stand_publishers_id'=> $standPublisher->id]) }}"--}}
                               @elseif(!$standPublisher && $canEdit)
...
@endif">
                        ...
                        @can('stand.make_entry')
                            <div class="modal fade" id="exampleModalFullscreen{{$day}}{{$time}}{{$entry['date']}}{{$StandTemplate->id}}" tabindex="-1" aria-labelledby="exampleModalFullscreenLabel"
                                 aria-hidden="true">
                                                            ...
                                                            <div class="d-flex align-items-center">
                                                                <div class="flex-grow-1">
                                                                    ...
                                                                    <form id="recordStandFirst" method="post" action="{{ route('NewRecordStand2') }}">
                                                                        ...
                                                                        <input type="hidden" name="date1" id="date" value="{{$entry['date']}}">
              ...
              "width": "20rem"
                            ...
                            @if($standPublisher && $canEdit)
                                <div class="modal fade" id="ModalFullscreenRedaction{{$day}}{{$time}}{{$entry['date']}}{{$standPublisher->id}}{{$StandTemplate->id}}" tabindex="-1" aria-labelledby="exampleModalFullscreenLabel"
                                     aria-hidden="true">
                                    <div class="modal-dialog modal-fullscreen-sm-down">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                        ...
                                                        <div class="card-body">
                                                            <div class="d-flex align-items-center mb-2">
                                                                <div class="flex-grow-1">
                                                                    ...
                                                                    <form id="changeForm{{ $i }}" method="post" action="{{ route('AddPublisherToStand2', ['id' => $standPublisher->id, 'stand' => $StandTemplate->stand_id, 'user_id' => $user->id]) }}">
                                                                        @csrf
                                                                        <div class="tom-select-custom">
                                                            ...
                                                            @else
                                                                @can('stand.delete_entry')
                                                                    <div class="col-12">
                                                                        <div class="d-grid gap-2">
                                                                            <a class="btn btn-outline-danger" type="button" href="{{ route('recordRedactionDelete2', ['id' => $standPublisher->id, 'stand' => $StandTemplate->stand_id, 'user_id' => $publishers[$userKey] ?? null]) }}"
...
<script>
    function openModal(day, time, gwe, StandTemplate) {
        // console.log('Day:', day);
        // console.log('Time:', time);
        // console.log('Gwe:', gwe);
        // console.log('StandTemplate:', StandTemplate);
        var modalId = 'exampleModalFullscreen' + day + time + gwe+ StandTemplate;
        var myModal = new bootstrap.Modal(document.getElementById(modalId));
        myModal.show();
    }

    function openModalRedaction(day, time, gwe, standPublisherId, StandTemplate) {
        // console.log('Day:', day);
        // console.log('Time:', time);
        // console.log('Gwe:', gwe);
        // console.log('StandPublisherId:', standPublisherId);
        // console.log('StandTemplate:', StandTemplate);
        var modalId = 'ModalFullscreenRedaction' + day + time + gwe + standPublisherId +StandTemplate;
        var myModal = new bootstrap.Modal(document.getElementById(modalId));
        myModal.show();
    }
</script>
